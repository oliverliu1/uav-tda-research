...
# ==============================================================================
# LOAD NETWORK MANIFOLD DATA AND TRAIN/TEST INDICES
# ==============================================================================
print("Loading Network manifold and split indices...")
...
try:
    ...
    print(f"✓ Memory usage: {network_data.memory_usage(deep=True).sum() / 1024**2:.2f} MB\n")
...
persistence_diagrams_network_train = diagrams_to_array(train_diagrams_raw, MAX_EDGE_LENGTH)
persistence_diagrams_network_test = diagrams_to_array(test_diagrams_raw, MAX_EDGE_LENGTH)

print(f"✓ Train diagrams shape: {persistence_diagrams_network_train.shape}")
print(f"✓ Test diagrams shape:  {persistence_diagrams_network_test.shape}")
...
# Train diagrams
np.save(
    f"{PERSISTENCE_DIR}/network_persistence_diagrams_train.npy",
    persistence_diagrams_network_train,
)
print(f"✓ Saved: {PERSISTENCE_DIR}/network_persistence_diagrams_train.npy")
file_size = os.path.getsize(
    f"{PERSISTENCE_DIR}/network_persistence_diagrams_train.npy"
) / (1024**2)
print(f"  File size: {file_size:.2f} MB")

with open(f"{PERSISTENCE_DIR}/network_gudhi_diagrams_train.pkl", "wb") as f:
    pickle.dump(train_diagrams_raw, f)
print(f"✓ Saved: {PERSISTENCE_DIR}/network_gudhi_diagrams_train.pkl")

# Test diagrams
np.save(
    f"{PERSISTENCE_DIR}/network_persistence_diagrams_test.npy",
    persistence_diagrams_network_test,
)
print(f"✓ Saved: {PERSISTENCE_DIR}/network_persistence_diagrams_test.npy")
file_size = os.path.getsize(
    f"{PERSISTENCE_DIR}/network_persistence_diagrams_test.npy"
) / (1024**2)
print(f"  File size: {file_size:.2f} MB")

with open(f"{PERSISTENCE_DIR}/network_gudhi_diagrams_test.pkl", "wb") as f:
    pickle.dump(test_diagrams_raw, f)
print(f"✓ Saved: {PERSISTENCE_DIR}/network_gudhi_diagrams_test.pkl")

# Metadata
metadata = {
    "manifold": "Network",
    "library": f"GUDHI {gd.__version__}",
    "n_train_samples": len(persistence_diagrams_network_train),
    "n_test_samples": len(persistence_diagrams_network_test),
...
for split_name, diagrams, split_indices in [
    ("TRAIN", persistence_diagrams_network_train, train_indices),
    ("TEST", persistence_diagrams_network_test, test_indices),
]:
...
print("=" * 80)
print("\nOutputs created:")
print(f"  - {PERSISTENCE_DIR}/network_persistence_diagrams_train.npy")
print(f"  - {PERSISTENCE_DIR}/network_persistence_diagrams_test.npy")
print(f"  - {PERSISTENCE_DIR}/network_gudhi_diagrams_train.pkl")
print(f"  - {PERSISTENCE_DIR}/network_gudhi_diagrams_test.pkl")
