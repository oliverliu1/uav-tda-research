...
# ==============================================================================
# LOAD C2 MANIFOLD DATA AND TRAIN/TEST INDICES
# ==============================================================================
print("Loading C2 manifold (Command & Control) and split indices...")
...
persistence_diagrams_c2_train = diagrams_to_array(train_diagrams_raw, MAX_EDGE_LENGTH)
persistence_diagrams_c2_test = diagrams_to_array(test_diagrams_raw, MAX_EDGE_LENGTH)

print(f"✓ Train diagrams shape: {persistence_diagrams_c2_train.shape}")
print(f"✓ Test diagrams shape:  {persistence_diagrams_c2_test.shape}")
...
# Train diagrams
np.save(
    f"{PERSISTENCE_DIR}/c2_persistence_diagrams_train.npy",
    persistence_diagrams_c2_train,
)
print(f"✓ Saved: {PERSISTENCE_DIR}/c2_persistence_diagrams_train.npy")
file_size = os.path.getsize(f"{PERSISTENCE_DIR}/c2_persistence_diagrams_train.npy") / (1024**2)
print(f"  File size: {file_size:.2f} MB")

with open(f"{PERSISTENCE_DIR}/c2_gudhi_diagrams_train.pkl", "wb") as f:
    pickle.dump(train_diagrams_raw, f)
print(f"✓ Saved: {PERSISTENCE_DIR}/c2_gudhi_diagrams_train.pkl")

# Test diagrams
np.save(
    f"{PERSISTENCE_DIR}/c2_persistence_diagrams_test.npy",
    persistence_diagrams_c2_test,
)
print(f"✓ Saved: {PERSISTENCE_DIR}/c2_persistence_diagrams_test.npy")
file_size = os.path.getsize(f"{PERSISTENCE_DIR}/c2_persistence_diagrams_test.npy") / (1024**2)
print(f"  File size: {file_size:.2f} MB")

with open(f"{PERSISTENCE_DIR}/c2_gudhi_diagrams_test.pkl", "wb") as f:
    pickle.dump(test_diagrams_raw, f)
print(f"✓ Saved: {PERSISTENCE_DIR}/c2_gudhi_diagrams_test.pkl")

# Metadata
metadata = {
    "manifold": "C2",
    "library": f"GUDHI {gd.__version__}",
    "n_train_samples": len(persistence_diagrams_c2_train),
    "n_test_samples": len(persistence_diagrams_c2_test),
...
for split_name, diagrams, split_indices in [
    ("TRAIN", persistence_diagrams_c2_train, train_indices),
    ("TEST", persistence_diagrams_c2_test, test_indices),
]:
...
print("=" * 80)
print("\nOutputs created:")
print(f"  - {PERSISTENCE_DIR}/c2_persistence_diagrams_train.npy")
print(f"  - {PERSISTENCE_DIR}/c2_persistence_diagrams_test.npy")
print(f"  - {PERSISTENCE_DIR}/c2_gudhi_diagrams_train.pkl")
print(f"  - {PERSISTENCE_DIR}/c2_gudhi_diagrams_test.pkl")
